              ...
              {pair.map(item => (
                <View key={item.id} style={{ flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 3 }}>
                  <View style={{ justifyContent: 'center', alignItems: 'center' }}>
                    <Image source={item.productImage} style={{ width: 172, height: 250, objectFit: 'contain' }} />
                  ...
                  <View>
                    <TouchableOpacity onPress={() => navigation.navigate('ProductDetailsScreen', { product: item })}>
                      <Text style={{ fontSize: 15, fontWeight: '500', marginTop: 3, width: 170 }}>{item.category}</Text>
                      <Text style={{ fontSize: 13, fontWeight: '400', marginBottom: 3, width: 170 }}>{item.description}</Text>
